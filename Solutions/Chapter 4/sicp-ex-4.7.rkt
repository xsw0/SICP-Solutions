...
(define (frame-variables frame) (map binding-variable frame))
(define (frame-values frame) (map binding-value frame))
(define (add-binding-to-frame! binding frame)
  (set-cdr! frame (cons (first-binding frame)
                        (last-bindings frame)))
  (set-car! frame binding))
(define (first-binding frame) (car frame))
(define (last-bindings frame) (cdr frame))
  ...
  (if (eq? frame the-empty-frame)
      ...
      (let ((binding (first-binding frame)))
        (if (eq? var (binding-variable binding))
            binding
            (lookup-variable-in-frame var (last-bindings frame))))))
...
(define (setup-environment)
  (let ((initial-env
         (extend-environment (map make-binding
